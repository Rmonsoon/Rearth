...
(function() {
    ...
    var NULL_WIND_VECTOR = [NaN, NaN, null];  // undefined location outside the vector field [u, v, mag]
    ...
    function validityDate(grids) {
        // When the active layer is considered "current", use its time as now, otherwise use current time as now (but rounded down to the nearest three-hour block).
        var now = grids ? grids.primaryGrid.date.getTime() : Math.floor(Date.now());
     ...
     * the attributes represented by newAttr. An event listener is  registered for configuration change events,
     * so when a change occurs the button becomes highlighted (".highlighted" is assigned or removed) if
     * the configuration matches the attributes for this button. The attributes used for the matching is taken
    ...
    function init() {
        ...
        if ("ontouchstart" in document.documentElement) {
			...
			// to filter styles problematic for touch
        }
